...
<div class="mb-4">
    ...
    <input type="text" name="introduction" value="{{ old('introduction', $about->introduction ?? '') }}"
...
<div class="mb-4">
    ...
    <input type="text" name="features" value="{{ old('features', $about->features ?? '') }}"
...
<div class="mb-4">
    ...
    <input type="file" name="images"
           class="w-full border p-2 rounded @error('images') border-red-500 @enderror">
    @error('images')
    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
    @enderror

    @if (!empty($about?->images))
        <img src="{{ asset('storage/' . $about->images) }}" class="w-24 h-24 mt-2 object-cover rounded shadow">
    @endif
</div>
